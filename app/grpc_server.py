...
from app.core.engine import acestep_engine
...
class AudioGatewayServicer(gateway_pb2_grpc.AudioGatewayServiceServicer):
    async def SubmitAudioJob(self, request, context):
        ...
        logger.info(f"Music Job Accepted: {request.prompt}", event_id="GRPC_JOB_ACCEPTED", trace_id=trace_id)
        
        # Asenkron üretim başlat
        asyncio.create_task(
            acestep_engine.generate_async(
